...
class FileUpload {
    ...
    public $addPrefix = false;            //(Optional) Add a random number prefix to the new uploaded file
    private $_actualPrefix = null;
    public $clearWhiteSpaces = false;     //(Optional) Clears white spaces from filename
    public $replaceText = '';
            
    function __construct($uploadName = null, $trackProgress = FALSE) {
        if ($uploadName !== null) {
            $this->uploadName = $uploadName;
        }
        
        //add the track progress variable
    ...
    public function setFilePrefix($prefix = null) {
        $this->addPrefix = true;
        
        if(!is_null($prefix))
            $this->_actualPrefix = $prefix;
    }
    
    public function clearWhiteSpaces($bool = TRUE, $replacetxt = '') {
        $this->clearWhiteSpaces = $bool;
    ...
    public function handleUpload($uploadDir = null, $allowedExtensions = null) {
        ...
        if($this->addPrefix == TRUE){
            
            if(!is_null($this->_actualPrefix)){
                $this->fileName = $this->_actualPrefix.$this->fileName;
            }
            else{
                $this->fileName = rand(0, 1000).'_'.$this->fileName;
            }
        }
        
        if($this->clearWhiteSpaces){
            $this->fileName = str_replace(' ', $this->replaceText, $this->fileName);
        }
